...
func JwtLogin(c echo.Context, api *ApiServer, user table.User, roles []table.UserRole) error {
	...
	if err != nil {
		return wr.NewError(wr.RespErrJwtAccessTokenParsing, errx.Wrapf(err, "unable to create access token for user (id: %d)", user.ID))
	}
	...
	if err != nil {
		return wr.NewError(wr.RespErrJwtRefreshTokenParsing, errx.Wrapf(err, "unable to create refresh token for user (id: %d)", user.ID))
	}
	...
	if err != nil {
		return wr.NewError(wr.RespErrJwtRefreshTokenCreate, errx.Wrapf(err, "unable to create refresh token database entry for user (id: %d)", user.ID))
	}
